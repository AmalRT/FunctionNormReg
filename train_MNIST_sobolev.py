...
def get_arguments():
    ...
    parser.add_argument("--results-dir", type=str, default='/esat/kochab/arannen/Pytorch_fun_regularization_sobolev',
                        help="Where to save snapshots of the model.")
    parser.add_argument("--expname", type=str, default='MNIST_300_1n_1_LeNet')
    ...
    parser.add_argument('--data-portion', type=int, default=300,
    ...
    parser.add_argument('--reg-batch-size', type=int, default=60,
    ...
    parser.add_argument('--VAE-batch-size', type=int, default=60,
...
def train(net, logger, start_iter, train_iterator, optimizer, next_stop, train_loader, args, netVAE=None, z_mu_all=None, z_var_all=None):
    ...
    for it in range(start_iter, next_stop+1):
        ...
        if args.function_norm:
            ...
            dataReg = netVAE.P(z)
            dataReg = Variable(dataReg.data.type(dtype), requires_grad=True)
            if dataReg.dim() == 2:
        ...
        if args.function_norm:
            #net.eval()
            reg = net.sobolev_norm(dataReg,dtype)
